...
(function(window, undefined) {
	...
	var jin = {
		...
		err: function(factor, errName, errRemark) {
			...
			if($fullerr.size()<1){
				$("body").append(
					'<div id="fullerr" style="background:#000;color:#FFF;font-size:12px;line-height:15px;'+
					'position:fixed;bottom:0;left:0;width:100%;height:200px;overflow:auto;"></div>'
				);
				$fullerr=$("#fullerr");
			}
			if(!factor()){
				var errHtml='';
				errHtml+=
					'<div style="background:#666;border:solid 2px #999;margin:5px;">'+
					'<div style="background:#999;font-weight:bold;">'+errName+':</div>';
				if(jin.type(errRemark)==="String"){
					errHtml+='<div>'+errRemark+'</div>';
				}else if(jin.type(errRemark)==="Array"){
					var len=errRemark.length;
					for(var i=0;i<len;i++){
						errHtml+='<div>'+errRemark[i]+'</div>';
					}
				}
				errHtml+='</div>';
				$fullerr.append(errHtml);
			}*/
		},
		...
		// fn:		获取url路由
		// author:	jin
		// return:	{object} 结果json对象
		// remark:	地址源：test.html#j-index,id-90
		//			解析后：{"j":"index","id","90"}
		getHash: function() {
		...
		// fn:		动态加载img文件
		// author:	jin
		// param:	{string} url img文件路径
		//			{function} callback img加载完毕回调
		//			{json} data 回调附带参数
		...
		getXHRObj: function() {
			...
			function() {
				return new ActiveXObject('Xml2.XMLHTTP');
			},
		...
		gapCheck: function(cbFactor, cb, tOut, tSplit) {
			if (cbFactor && cb) {
				...
				function factor() {
					if (jin.type(cbFactor) == "Boolean") {
						return cbFactor;
					} else if (jin.type(cbFactor) == "Function") {
						return cbFactor();
					}
		...
		// return:	{boolean} 对象是否存在
		// example:	
		//			jin.objAttrCheck(o,"result.list");  //o.result.list
		...
		wScrollWidth: function(r) {
			var d = document;
			if (!jin._scrollWidth || r) {
				jin._scrollWidth = d.documentElement.scrollWidth;
			}
			return jin._scrollWidth;
		},
		// fn:		scrollHeight
		// author:	jin
		// param:	{boolean} r 是否刷新当前数据
		...
		wScrollHeight: function(r) {
			var d = document;
			if (!jin._scrollHeight || r) {
				jin._scrollHeight = d.documentElement.scrollHeight;
			}
			return jin._scrollHeight;
		},
		...
		// fn:		动画滚屏
		// author:	jin
		// param:	{number} r 滚动到目标位置
		//			{function} [fnBefore] 动画开始前回调
		//			{function} [fnAfter] 动画结束后回调
		// relation:
		//			jin.Quad.easeOut
		//			jin.wScrollTop
		anScroll: function(r, fnBefore, fnAfter) {
			fnBefore && fnBefore();
			var b = jin.wScrollTop(),
				c = r - jin.wScrollTop(),
				d = 40,
				t = 0;
			var run = function() {
